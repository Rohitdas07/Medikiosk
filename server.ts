...
import { extractPrescriptionRules, normalizeMedicineName, isKnownMedicineName, sanitizeMedicationList } from './src/utils/prescriptionRuleExtractor';
...
const OSM_ROUTING_MIRROR_URL = (process.env.OSM_ROUTING_MIRROR_URL || 'https://routing.openstreetmap.de').replace(/\/$/, '');
...
if (!SUPABASE_URL || !SUPABASE_SERVER_KEY) {
  console.warn('[Supabase Server] Missing SUPABASE_URL/SUPABASE_ANON_KEY. Server-side Supabase features will be unavailable until configured.');
}

let supabaseServer: any = null;
try {
  supabaseServer = createClient(SUPABASE_URL, SUPABASE_SERVER_KEY, {
...
function isUsableExtractedField(value: any): boolean {
  if (value === undefined || value === null) return false;
  const v = String(value).trim();
  return !!v && !/^(?:unclear|unknown|not detected|as prescribed|as directed)$/i.test(v);
}

function medicationKey(name: string): string {
  return normalizeMedicineName(name).toLowerCase().replace(/[^a-z0-9]+/g, '');
}

/**
 * Local OCR is used as the factual anchor for medicine rows. Gemini can fill
 * missing fields, but it must not replace a medicine that local OCR already
 * identified with an unrelated hallucinated name.
 */
function mergeMedicationExtractions(localMeds: any[], aiMeds: any[]): any[] {
  const local = sanitizeMedicationList(localMeds || []);
  const ai = sanitizeMedicationList(aiMeds || []);
  const usedAi = new Set<number>();

  const merged = local.map((localMed: any, index: number) => {
    const localName = normalizeMedicineName(localMed.medicineName || '');
    let matchIndex = -1;
    let bestScore = -1;

    ai.forEach((candidate: any, aiIndex: number) => {
      if (usedAi.has(aiIndex)) return;
      const candidateName = normalizeMedicineName(candidate.medicineName || '');
      let score = 0;
      if (medicationKey(localName) && medicationKey(localName) === medicationKey(candidateName)) score = 100;
      else if (candidateName && candidateName.toLowerCase() === localName.toLowerCase()) score = 95;
      else if (index === aiIndex) score = 10;
      if (score > bestScore) {
        bestScore = score;
        matchIndex = aiIndex;
      }
    ...
    const aiMed = matchIndex >= 0 && bestScore >= 90 ? ai[matchIndex] : null;
    if (aiMed) usedAi.add(matchIndex);

    const normalizedAiName = normalizeMedicineName(aiMed?.medicineName || '');
    return {
      ...localMed,
      id: aiMed?.id || localMed.id,
      medicineName: localName || normalizedAiName || localMed.medicineName,
      strength: isUsableExtractedField(localMed.strength) ? localMed.strength : (aiMed?.strength || ''),
      dosage: isUsableExtractedField(localMed.dosage) ? localMed.dosage : (aiMed?.dosage || 'Unclear'),
      frequency: isUsableExtractedField(localMed.frequency) ? localMed.frequency : (aiMed?.frequency || 'As prescribed'),
      duration: isUsableExtractedField(localMed.duration) ? localMed.duration : (aiMed?.duration || 'As directed'),
      route: isUsableExtractedField(localMed.route) ? localMed.route : (aiMed?.route || 'Oral'),
      timing: isUsableExtractedField(localMed.timing) ? localMed.timing : (aiMed?.timing || 'As directed'),
      foodInstruction: isUsableExtractedField(localMed.foodInstruction) ? localMed.foodInstruction : (aiMed?.foodInstruction || 'As directed'),
      specialInstruction: localMed.specialInstruction || aiMed?.specialInstruction || '',
      confidenceScore: Math.max(Number(localMed.confidenceScore || 0), Number(aiMed?.confidenceScore || 0)),
      isLowConfidence: Boolean(localMed.isLowConfidence) || Boolean(aiMed?.isLowConfidence),
      patientVerified: false
    };
  });

  // Append only AI medicines that resolve to a known/canonical medicine and are
  // genuinely new. This prevents outputs such as "Cobimzbue" from becoming a
  // second false medicine when local OCR already identified Cetirizine.
  ai.forEach((candidate: any, aiIndex: number) => {
    if (usedAi.has(aiIndex)) return;
    const canonical = normalizeMedicineName(candidate.medicineName || '');
    const isKnown = isKnownMedicineName(canonical);
    const duplicate = merged.some((m: any) => medicationKey(m.medicineName) === medicationKey(canonical));
    if (isKnown && !duplicate) {
      merged.push({
        ...candidate,
  ...
  return sanitizeMedicationList(merged);
}

function parsePrescriptionDeterministic(ocrText: string, patientName: string = ''): any {
  const extracted = extractPrescriptionRules(ocrText || '', patientName);
  return {
    ...extracted,
    isReadable: Boolean((ocrText || '').trim()),
    isAiVerified: false,
    aiVerificationStatus: 'unavailable',
    hasLowConfidenceFields: extracted.medications.some((m: any) => m.isLowConfidence) || extracted.medications.length === 0,
    clinicalSummary: extracted.clinicalSummary || `Prescription extracted with ${extracted.medications.length} medication(s).`
  };
}
...
app.post('/api/prescriptions', (req, res) => {
  // Accept both the direct PrescriptionRecord payload and the scanner's
  // { prescription, auditContext } envelope.
  const prescription = req.body?.prescription || req.body;
  if (!prescription.id) {
...
app.post('/api/prescriptions/ocr-ai-scan', async (req, res) => {
  try {
    ...
    // Never make a network call with an empty API key. This also makes the
    // local VS Code demo work without Gemini credentials.
    if (!process.env.GEMINI_API_KEY) {
      const detData = parsePrescriptionDeterministic(ocrText, patientName);
      return res.json({ success: true, isAiVerified: false, aiUnavailable: true, extractedData: detData });
    }
...
Output format MUST be strictly valid JSON matching this exact structure:
{
  ...
  "medications": [
    ...
    // If Gemini answered with an empty array, preserve medicines detected by
    // local OCR/rules instead of showing '0 medicines'.
    if ((!Array.isArray(parsed.medications) || parsed.medications.length === 0) && ocrText) {
      const deterministic = parsePrescriptionDeterministic(ocrText, patientName);
      if (deterministic.medications.length > 0) {
        parsed.medications = deterministic.medications;
        parsed.ocrText = parsed.ocrText || deterministic.ocrText;
        parsed.hasLowConfidenceFields = false;
      }
    }

    // Reconcile Gemini against the raw OCR before returning anything to the UI.
    // The local OCR row parser is the source of truth for medicine identity and
    // any dosage/frequency/duration it can actually read. Gemini only fills gaps.
    const deterministic = parsePrescriptionDeterministic(ocrText || parsed.ocrText || '', patientName);
    parsed.medications = mergeMedicationExtractions(deterministic.medications, parsed.medications || []);
    parsed.isAiVerified = true;
    parsed.aiVerificationStatus = 'verified';
    if (!parsed.diagnosis || /^diagnosis not explicitly mentioned/i.test(parsed.diagnosis)) parsed.diagnosis = deterministic.diagnosis;
    if (!parsed.symptoms || /^no symptoms detected/i.test(parsed.symptoms)) parsed.symptoms = deterministic.symptoms;
...
app.post('/api/hospitals/search', async (req, res) => {
  try {
    ...
    let nominatimUrl = `${OSM_NOMINATIM_URL}/search?format=json&q=${encodeURIComponent(cleanQuery)}&limit=25&addressdetails=1`;
    ...
    const osmResponse = await fetch(nominatimUrl, {
      headers: { 'User-Agent': process.env.OSM_USER_AGENT || 'MediKioskAI/1.0 (+https://medikiosk.app; contact: admin@medikiosk.app)' },
...
app.post('/api/routes/compute', async (req, res) => {
  try {
    ...
    // -------------------------------------------------------------
    const osrmMirrors = [
      `${OSM_ROUTING_PRIMARY_URL}/route/v1/${osrmProfile}/${origin.lng},${origin.lat};${destination.lng},${destination.lat}?overview=full&geometries=geojson&steps=true`,
      `${OSM_ROUTING_MIRROR_URL}/routed-${deProfile}/route/v1/${osrmProfile}/${origin.lng},${origin.lat};${destination.lng},${destination.lat}?overview=full&geometries=geojson&steps=true`
    ];
...
app.post('/api/geocode/reverse', async (req, res) => {
  try {
    ...
    try {
      const osmRes = await fetch(`${OSM_NOMINATIM_URL}/reverse?format=json&lat=${lat}&lon=${lng}&zoom=14&addressdetails=1`, {
        headers: {
          'User-Agent': process.env.OSM_USER_AGENT || 'MediKioskAI/1.0 (+https://medikiosk.app; contact: admin@medikiosk.app)'
        },
...
app.post('/api/geocode/forward', async (req, res) => {
  try {
    ...
    try {
      const osmRes = await fetch(`${OSM_NOMINATIM_URL}/search?format=json&q=${encodeURIComponent(cleanQuery + ', India')}&limit=1&addressdetails=1`, {
        headers: {
          'User-Agent': process.env.OSM_USER_AGENT || 'MediKioskAI/1.0 (+https://medikiosk.app; contact: admin@medikiosk.app)'
        },
        signal: AbortSignal.timeout(6000)
...
const MASTER_HIS_ADMIN = {
  adminId: process.env.HIS_ADMIN_ID || '',
  pinHash: process.env.HIS_ADMIN_PIN ? hashStaffPin(process.env.HIS_ADMIN_PIN) : '',
...
// Demo staff credentials are disabled by default in production.
// Real deployments should provision staff records in Supabase and use the server-side
// service/secret key for privileged staff lookups.
if (process.env.ENABLE_DEMO_STAFF !== 'true') {
  HOSPITAL_STAFF_STORE = [];
}
...
const requireAdminAuth = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  const isAuthorized = Boolean(token) && ACTIVE_ADMIN_SESSIONS.has(token);
...
app.post('/api/auth/his-admin-login', (req, res) => {
  ...
  if (!MASTER_HIS_ADMIN.adminId || !MASTER_HIS_ADMIN.pinHash) {
    return res.status(503).json({ success: false, error: 'HIS Administrator authentication is not configured on this deployment.' });
  }
...
// -------------------------------------------------------------
// PATIENT ACCOUNT AUTHENTICATION (shared across devices)
// -------------------------------------------------------------
// Patient PINs are stored only as salted scrypt hashes in a dedicated table.
// The raw PIN is never persisted or returned to the browser.
function hashPatientPin(pin: string): string {
...
function verifyPatientPin(pin: string, encodedHash: string): boolean {
  try {
    ...
    const actual = crypto.scryptSync(String(pin || '').trim(), salt, expected.length);
...
function normalizePatientIdentifier(value: unknown): { value: string; type: 'email' | 'aadhaar' | 'abha' | 'other' } {
  const trimmed = String(value || '').trim();
  const digits = trimmed.replace(/\D/g, '');
  if (trimmed.includes('@')) return { value: trimmed.toLowerCase(), type: 'email' };
  if (/^\d{12}$/.test(digits)) return { value: digits, type: 'aadhaar' };
  if (/^\d{14}$/.test(digits)) return { value: digits, type: 'abha' };
  return { value: trimmed, type: 'other' };
}

async function findPatientForAuthentication(identifier: string) {
  ...
  if (normalized.type === 'email') {
    dbQuery = dbQuery.ilike('email', normalized.value);
  } else if (normalized.type === 'aadhaar') {
    // Staging/demo only: the current schema intentionally stores Aadhaar last4,
    // not the full Aadhaar number. Full UIDAI authentication is not implemented.
    dbQuery = dbQuery.eq('aadhaar_last4', normalized.value.slice(-4));
  } else if (normalized.type === 'abha') {
    dbQuery = dbQuery.eq('abha_id', normalized.value);
  } else {
    dbQuery = dbQuery.or(`uhid.eq.${normalized.value},patient_id.eq.${normalized.value}`);
  }
...
app.post('/api/auth/patient-register', async (req, res) => {
  if (!supabaseServer || !SUPABASE_SERVICE_ROLE_KEY) {
    return res.status(503).json({
      success: false,
      error: 'Patient authentication backend is not configured. Add SUPABASE_SERVICE_ROLE_KEY to the Render server environment.'
    });
  }

  const { patient, pin } = req.body || {};
  const cleanPin = String(pin || '').trim();
  if (!patient || !patient.id || cleanPin.length < 4 || cleanPin.length > 6 || !/^\d+$/.test(cleanPin)) {
    return res.status(400).json({ success: false, error: 'A valid patient profile and 4-6 digit PIN are required.' });
  }

  try {
    const existing = await findPatientForAuthentication(patient.email || patient.abhaId || patient.aadhaarLast4 || patient.uhid || patient.id);
    if (existing && existing.id !== patient.id) {
      return res.status(409).json({ success: false, error: 'A patient account with these registration details already exists.' });
    }
    ...
    const patientPayload = {
      ...
      aadhaar_last4: patient.aadhaarLast4 || null,
      aadhaar_verification_status: patient.aadhaarVerificationStatus || 'not_verified',
      aadhaar_verification_ref: patient.aadhaarVerificationRef || null,
      consent_given: patient.consentGiven ?? false,
      ...
      queue_token: patient.tokenNumber || '',
      is_emergency: patient.triageRisk === 'CRITICAL_EMERGENCY',
      ayush_assessment: patient.ayushAssessment || null,
      clinical_summary: patient.clinicalSummary || null,
      updated_at: new Date().toISOString()
    };

    const { data: savedPatient, error: patientError } = await supabaseServer
      .from('patients')
    ...
    const { error: authError } = await supabaseServer
      .from('patient_auth')
      .upsert([{
        patient_id: savedPatient.id,
        pin_hash: hashPatientPin(cleanPin),
        updated_at: new Date().toISOString()
      }], { onConflict: 'patient_id' });
    if (authError) throw new Error(authError.message);

    return res.json({ success: true, patient: savedPatient });
  } catch (error: any) {
    console.error('[Patient Register] Error:', error?.message);
    return res.status(500).json({ success: false, error: 'Unable to create the patient account.' });
  }
});
...
app.post('/api/auth/patient-login', async (req, res) => {
  ...
  if (!identifier || !/^\d{4,6}$/.test(pin)) {
    return res.status(400).json({ success: false, error: 'Enter your registered identifier and 4-6 digit PIN.' });
  }

  try {
    const patient = await findPatientForAuthentication(identifier);
    if (!patient) {
      return res.status(401).json({ success: false, error: 'No registered patient account was found for those details.' });
    }

    const { data: authRow, error: authError } = await supabaseServer
      .from('patient_auth')
      .select('patient_id, pin_hash, disabled')
      .eq('patient_id', patient.id)
      .maybeSingle();
    if (authError) throw new Error(authError.message);
    if (!authRow || authRow.disabled || !verifyPatientPin(pin, authRow.pin_hash)) {
      return res.status(401).json({ success: false, error: 'Incorrect PIN. Please verify your PIN and try again.' });
    }
