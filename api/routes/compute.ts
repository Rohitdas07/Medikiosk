...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const mirrors = [
      `${(process.env.OSM_ROUTING_PRIMARY_URL || 'https://router.project-osrm.org').replace(/\/$/, '')}/route/v1/${osrmProfile}/${origin.lng},${origin.lat};${destination.lng},${destination.lat}?overview=full&geometries=geojson&steps=true`,
      `${(process.env.OSM_ROUTING_MIRROR_URL || 'https://routing.openstreetmap.de').replace(/\/$/, '')}/routed-${deProfile}/route/v1/${osrmProfile}/${origin.lng},${origin.lat};${destination.lng},${destination.lat}?overview=full&geometries=geojson&steps=true`
    ];
