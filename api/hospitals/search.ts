...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    let nominatimUrl = `${(process.env.OSM_NOMINATIM_URL || 'https://nominatim.openstreetmap.org').replace(/\/$/, '')}/search?format=json&q=${encodeURIComponent(cleanQuery)}&limit=25&addressdetails=1`;
