...
async function discoverFacilities(
  ...
  try {
    const nominatimQueries = [
      `${(process.env.OSM_NOMINATIM_URL || 'https://nominatim.openstreetmap.org').replace(/\/$/, '')}/search?format=json&q=hospital&viewbox=${west},${north},${east},${south}&bounded=1&limit=50&addressdetails=1`
    ];
    if (!hospitalsOnly && maxRadiusKm <= 25) {
      nominatimQueries.push(
        `${(process.env.OSM_NOMINATIM_URL || 'https://nominatim.openstreetmap.org').replace(/\/$/, '')}/search?format=json&q=clinic&viewbox=${west},${north},${east},${south}&bounded=1&limit=30&addressdetails=1`
      );
    }
