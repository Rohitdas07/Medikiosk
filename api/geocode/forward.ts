  ...
  try {
    ...
    try {
      const osmRes = await fetch(
        `${(process.env.OSM_NOMINATIM_URL || 'https://nominatim.openstreetmap.org').replace(/\/$/, '')}/search?format=json&q=${encodeURIComponent(cleanQuery + ', India')}&limit=1&addressdetails=1`,
        {
          headers: {
