...
export async function computeLiveRoute(
  ...
  try {
    ...
    const clientMirrors = [
      `${(import.meta as any).env?.VITE_OSM_ROUTING_PRIMARY_URL || 'https://router.project-osrm.org'}/route/v1/${osrmProfile}/${origin.lng},${origin.lat};${destination.lng},${destination.lat}?overview=full&geometries=geojson&steps=true`,
      `${(import.meta as any).env?.VITE_OSM_ROUTING_MIRROR_URL || 'https://routing.openstreetmap.de'}/routed-${deProfile}/route/v1/${osrmProfile}/${origin.lng},${origin.lat};${destination.lng},${destination.lat}?overview=full&geometries=geojson&steps=true`
    ];
