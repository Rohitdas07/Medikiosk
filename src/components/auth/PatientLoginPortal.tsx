...
import { saveAppointmentToSupabase } from '../../utils/supabaseClient';
...
export const PatientLoginPortal: React.FC<PatientLoginPortalProps> = ({
  ...
  const handleSwitchToOtp = () => {
    setErrorMessage('OTP sign-in is not enabled in this staging build. Please use your 4-6 digit PIN.');
  };
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedInput) {
      setErrorMessage('Please enter your registered Email address, Aadhaar number, or ABHA ID.');
      return;
    }

    const isEmail = trimmedInput.includes('@');
    const cleanDigits = trimmedInput.replace(/\D/g, '');
    const isAadhaar = /^\d{12}$/.test(cleanDigits);
    const isAbha = /^\d{14}$/.test(cleanDigits);
    ...
    } else if (!isAadhaar && !isAbha) {
      setErrorMessage('Please enter a valid email address, 12-digit Aadhaar number, or 14-digit ABHA ID.');
      return;
    }

    if (authMethod === 'pin' && (!loginPin.trim() || loginPin.trim().length < 4)) {
      setErrorMessage('Please enter your 4-6 digit security PIN.');
      return;
    }
    if (authMethod === 'otp' && otpDigits.join('').length < 4) {
      setErrorMessage(isEmail
        ? 'Please enter the 4-digit verification code sent to your email.'
        : 'Please enter the 4-digit verification code for your registered health identity.');
      return;
    }
    ...
    try {
      // Authenticate against the server so the same patient account can sign in
      // from any device. The server verifies the PIN and returns the shared patient record.
      const authResponse = await fetch('/api/auth/patient-login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ identifier: trimmedInput, pin: authMethod === 'pin' ? loginPin.trim() : otpDigits.join('') })
      });
      const authData = await authResponse.json();

      if (!authResponse.ok || !authData.success || !authData.patient || !authData.session) {
        setErrorMessage(authData.error || 'Unable to sign in. Please check your details and try again.');
        return;
      }

      const row = authData.patient;
      const matched: PatientProfile = {
        ...(row as any),
        id: row.id || row.patient_id,
        ...
        aadhaarVerificationStatus: row.aadhaar_verification_status || 'not_verified',
        aadhaarVerificationRef: row.aadhaar_verification_ref || undefined,
        language: row.language || 'en',
        careStream: row.care_stream || 'allopathy',
        department: row.department || 'General Medicine OPD',
        symptoms: row.symptoms || [],
        vitals: row.vitals || {},
        currentMedications: row.current_medications || [],
        allergies: row.allergies || [],
        pastIllnesses: row.past_illnesses || [],
        pastSurgeries: row.past_surgeries || [],
        familyHistory: row.family_history || [],
        timeline: row.timeline || [],
        triageRisk: row.triage_risk || 'STANDARD_OPD',
        redFlagsDetected: row.red_flags_detected || [],
        scannedDocuments: [],
      ...
      const newSession: AuthSession = {
        role: 'patient',
        userId: authData.session.userId || matched.id,
        userName: authData.session.userName || matched.name,
        patientId: authData.session.patientId || matched.id,
        token: authData.session.token,
        loginTime: authData.session.loginTime || new Date().toISOString()
      };
      ...
      setTimeout(() => {
        ...
        if (onLoginSuccess) onLoginSuccess(newSession, matched);
        else onNavigateView(target);
      }, 500);
    } catch (err: any) {
      setIsAuthenticating(false);
  ...
  const handleCreateAccountSubmit = async (e: React.FormEvent) => {
    ...
    const cleanAbha = optionalAbhaId.replace(/\D/g, '');
    if (cleanAbha && cleanAbha.length !== 14) {
      setErrorMessage('ABHA ID must contain exactly 14 digits.');
      return;
    }
    ...
    if (cleanAadhaar && cleanAadhaar.length !== 12) {
      setErrorMessage('Please enter a valid 12-digit Aadhaar number, or leave it blank.');
      return;
    }
    ...
    const newPatientProfile: PatientProfile = {
      ...
      aadhaarVerificationStatus: cleanAadhaar ? 'demo_verified' : 'not_verified',
      aadhaarVerificationRef: cleanAadhaar ? `DEMO-AADHAAR-${Date.now()}` : undefined,
      uhid: uhidNum,
      ...
      abhaId: cleanAbha || `${Math.floor(10 + Math.random() * 90)}${Math.floor(100000000000 + Math.random() * 900000000000)}`,
      consentGiven: true,
      consentType: 'touch',
    ...
    setTimeout(async () => {
      // 1. Register the account through the server. The raw PIN never leaves this request
      // as persisted data; the server stores only a salted scrypt hash.
      const registrationResponse = await fetch('/api/auth/patient-register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ patient: newPatientProfile, pin: createPin.trim() })
      });
      const registrationData = await registrationResponse.json();
      if (!registrationResponse.ok || !registrationData.success) {
        setIsAuthenticating(false);
        setErrorMessage(registrationData.error || 'Unable to create the patient account.');
        return;
      }
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm max-w-lg mx-auto space-y-6">
          ...
          <div className="text-center space-y-2 border-b border-slate-100 pb-5">
            ...
            <p className="text-xs text-slate-500 max-w-xs mx-auto">
              Enter your registered email address, Aadhaar number, or ABHA ID to access your health portal.
            </p>
          </div>

          <form onSubmit={handleSignInSubmit} className="space-y-4">
            {/* Email / Aadhaar / ABHA ID Field */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-700 block">
                  Email Address, Aadhaar or ABHA ID
                </label>
                {loginMobile.trim() && (
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-600">
                    {loginMobile.includes('@') ? 'Email' : /^\d{12}$/.test(loginMobile.replace(/[^0-9]/g, '')) ? 'Aadhaar' : 'ABHA ID'}
                  </span>
                )}
              </div>
              <div className="relative">
                <input
                  ...
                  placeholder="Enter email, 12-digit Aadhaar or ABHA ID"
                  required
              ...
              <p className="text-[11px] text-slate-500">
                Supports registered email address, 12-digit Aadhaar number, or ABHA ID.
              </p>
            </div>
                  ...
                  <div className="p-3 bg-teal-50/70 border border-teal-200 rounded-xl text-[11px] text-teal-900 flex items-start gap-2 animate-in fade-in">
                    ...
                    <div>
                      ...
                      <p className="text-teal-800">
                        PIN reset/OTP recovery is not enabled in this staging build.
                      </p>
                    </div>
                  </div>
                )}
      ...
      {activeTab === 'create-account' && (
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">
                ABHA ID <span className="text-slate-400 font-normal">(Optional • Exactly 14 digits)</span>
              </label>
              <input
                type="text"
            ...
            {/* Aadhaar Number - demo registration field */}
            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-slate-700 block mb-1">
                Aadhaar Number <span className="text-slate-400 font-normal">(Optional • Demo verification)</span>
              </label>
              <input
                type="password"
                inputMode="numeric"
                id="create-patient-aadhaar"
                value={aadhaarNumber}
              ...
              <p className="text-[11px] text-amber-700 mt-1">
                Demo only: do not use real Aadhaar numbers. No OTP/UIDAI verification is performed.
              </p>
            </div>
