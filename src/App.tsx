...
export function App() {
  ...
  const handleLoginSuccess = (session: AuthSession, patientProfile?: PatientProfile) => {
    ...
    if (session.role === 'patient') {
      ...
      // A patient authenticated on a new device is not present in that browser's
      // local kiosk cache yet. Merge the server-returned profile into React state
      // so the dashboard can render immediately on every device.
      if (patientProfile) {
        setPatients(prev => {
          const exists = prev.some(p => p.id === patientProfile.id);
          const updated = exists
            ? prev.map(p => p.id === patientProfile.id ? patientProfile : p)
            : [patientProfile, ...prev];
          saveKioskDataToStorage(updated, patientProfile.id);
          return updated;
        });
      }
