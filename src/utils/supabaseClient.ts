...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  console.warn('[MediKiosk] Supabase environment variables are missing. Configure VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY before production deployment.');
}
...
export async function savePatientProfileToSupabase(patient: PatientProfile): Promise<{ success: boolean; data?: any; error?: string; isDbPersisted?: boolean }> {
  try {
    const payload = {
      ...
      aadhaar_last4: (patient as any).aadhaarLast4 || null,
      aadhaar_verification_status: (patient as any).aadhaarVerificationStatus || 'not_verified',
      aadhaar_verification_ref: (patient as any).aadhaarVerificationRef || null,
      consent_given: (patient as any).consentGiven ?? false,
...
export async function lookupPatientInSupabase(query: string): Promise<{ patient?: any; error?: string }> {
  try {
    ...
    if (isEmail) {
      ...
      // Demo mode only: registration stores Aadhaar last four digits.
      // Do not treat this as real UIDAI authentication.
      dbQuery = dbQuery.eq('aadhaar_last4', cleanDigits.slice(-4));
    } else if (/^\d{14}$/.test(cleanDigits)) {
      // ABHA IDs are stored as exactly 14 digits by the database constraint.
      // Do not search the Aadhaar last-four field for a 14-digit ABHA input.
      dbQuery = dbQuery.eq('abha_id', cleanDigits);
    } else {
      dbQuery = dbQuery.or(`abha_id.eq.${trimmed},abha_id.eq.${cleanDigits},uhid.eq.${trimmed},patient_id.eq.${trimmed}`);
    }
